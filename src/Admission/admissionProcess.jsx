...
export default function Admissionprocess() {
  ...
  const getAdmissionData = async () => {
    axios
      .get(`${import.meta.env.VITE_APP_URL}api/user/admission-process`)
      .then((res) => {
        setAdmissionProcessData(res?.data?.data);
  ...
  useEffect(() => {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            {/* Hero Section */}
            <section className="relative h-120 bg-cover bg-center  rounded-bl-[45px] rounded-br-[45px] overflow-hidden" style={{ backgroundImage: `url(${import.meta.env.VITE_APP_URL}${admisssionProcessData?.banner})` }}>
             ...
             [box-shadow:0_2px_6px_2px_rgba(60,64,67,0.15),0_1px_2px_0_rgba(60,64,67,0.30)]"
                >
                    <div className="mb-8 text-[#333333] text-[18px] tracking-normal text-center font-['poppins'] font-medium">
                        <p className="mb-6">
                            {
                                admisssionProcessData?.academicYear
                            }
                        </p>
                    </div>

{admisssionProcessData?.section?.length > 0 &&
  admisssionProcessData?.section?.map((ele, sectionIndex) => (
